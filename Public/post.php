...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    for ($i = 0; $i < sizeof($array_of_name); $i++) {
        ...
        $sql_avs = "INSERT INTO detail_avs_mcc_7 (name, ampere, temperature) VALUES (?, ?, ?)";
        $new_statement = $new_connection->prepare($sql_avs);

        if ($ampere_input > $allowed_max_ampere or $ampere_input < 0) {
            $error = new Exception("Cek kembali ampere " . "baris " . $i + 1 . " yang anda input <br/>");
            echo $error->getMessage();

            $new_connection->exec("ROLLBACK");
            return;
        } else if ($temperature_input > $allowed_max_temperature or $ampere_input < 0) {
            $error = new Exception("Cek kembali temperature " . "baris " . $i + 1 . " yang anda input <br/>");
            echo $error->getMessage();
            ...
            echo "berhasil input " . $array_of_name[$i] . "<br/>";
        }
    }
